...
unsigned char slewLimiterPitchRequest = 0; /* The proposed pitch CV output of the slew limiter, in 12 bits */
unsigned char slewLimiterLag = 0; /* Whether the slew limiter should lag or not */
unsigned long slewLimiterThisTime = 0; /* Now, in milliseconds */
...
/*
 *  Slew limiter functions
 */

/* Make sure slewLimiterPitchRequest and slewLimiterLag are set appropriately first */
void slewLimiterSetPitch()
{
	slewLimiterThisTime = millis();
	slewLimiterInterval = slewLimiterThisTime - slewLimiterLastTime;
	slewLimiterLastTime = slewLimiterThisTime;

	if (slewLimiterLag == LOW) {
		slewLimiterPitch = slewLimiterPitchRequest;
	} else {
		if (slewLimiterPitchRequest > slewLimiterPitch + (slewLimiterInterval * 2)) {
			slewLimiterPitch = slewLimiterPitch + slewLimiterInterval;
		} else if (slewLimiterPitchRequest < slewLimiterPitch - (slewLimiterInterval * 2)) {
			slewLimiterPitch = slewLimiterPitch - slewLimiterInterval;
		} else {
			slewLimiterPitch = slewLimiterPitchRequest;
		}
